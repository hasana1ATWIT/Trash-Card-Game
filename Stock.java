...
public final class Stock extends Pile
    ...
    /**
     * @param currentCard
     *     the card drawn from stock to be evaluated
     * @param playerHand
     *     the hand of the current player
     *
     * @return true if card may be placed into player's hand, false otherwise
     *
     * @since 1.0
     */
    public boolean isUsable( final Card currentCard,
                             final Hand playerHand )
        {

        if ( ( currentCard.rank == KING ) || ( currentCard.rank == QUEEN ) )
            {
            return false ;
            }

        return playerHand.isSlotAvailable( currentCard ) ;
    ...
    public static void main( final String[] args )
        ...
        final Card topCard = myStock.drawTopCard() ;
        System.out.println( topCard.rank ) ;
        System.out.println( topCard.getOrientation() ) ;


        /***
         * try { System.out.printf( "add null to myStock:%n" ) ; myStock.addToTop( null ) ; } catch
         * ( Exception e ) { System.out.printf( "\t%s%n", e ) ; } try { System.out.printf( "add a
         * temporary card to myStock:%n" ) ; myStock.addToTop( new Card( JOKER ) ) ; } catch (
         * Exception e ) { System.out.printf( "\t%s%n", e ) ; }
         *///
